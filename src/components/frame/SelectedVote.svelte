<script>
  ...
  import Progress from '$components/icons/Progress.svelte';

  let isSelected = false;
  let selectedOption = true;

  const handleSelected = () => (isSelected = !isSelected);
</script>
...
<div class="container">
  <div class="title-container">
    <div class="title-wrapper">
      <div class="title-rec" />
      <p>나는 솔직히 잘 생겼다고 생각한다</p>
    </div>
    {#if isSelected}
      <IconButton on:click={handleSelected}>
        <ArrowUpIcon />
      </IconButton>
    {:else}
      <IconButton on:click={handleSelected}>
  ...
  {#if isSelected}
    ...
    <div class="progress-title-container">
      <p class="progress-positive-title">66.78%</p>
      <p class="progress-negative-title">34.22%</p>
    </div>
...
<style>
  ...
  :global(.progress-container :nth-child(2)) {
    margin-left: 6px;
  }
  ...
  .progress-positive-title {
    color: #2e4558;
  }
  .progress-negative-title {
    color: #f5ba62;
  }
  .answer-container {
    display: flex;
